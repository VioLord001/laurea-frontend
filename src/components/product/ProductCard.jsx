...
export default function ProductCard({ product }) {
  ...
  return (
    <Link href={`/products/${product.slug}`} style={{textDecoration:'none',color:'inherit'}}>
      <div style={{border:'1px solid #e0d8cc',borderRadius:'8px',overflow:'hidden',cursor:'pointer'}}>
        ...
        <div style={{padding:'12px'}}>
          ...
          <p style={{fontSize:'13px',color:'#2a1e10',marginBottom:'8px'}}>{product.name}</p>
          <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
            <span style={{fontSize:'14px',fontWeight:'600'}}>${parseFloat(product.price).toFixed(2)}</span>
